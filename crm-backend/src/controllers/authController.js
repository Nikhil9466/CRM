  ...
  try {
    ...
    const user = await prisma.$transaction(async tx => {
      if (await tx.organisation.findUnique({ where: { id: orgId } }))
        fail("This organisation already exists. Ask its administrator to create your account.", 409);
      await tx.organisation.create({ data: { id: orgId } });
      await tx.stage.createMany({ data: stages(orgId) });
      return tx.user.create({ data: { ...fields, orgId, passwordHash, role: "ADMIN" }, select: publicSelect });
    });
...
async function login(req, res, next) {
  try {
    ...
    if (typeof req.body.password !== "string" || !req.body.password || req.body.password.length > 1000)
      fail("Password is required.");
    const user = await prisma.user.findUnique({ where: { email } });
    if (!user?.active || !(await bcrypt.compare(req.body.password, user.passwordHash)))
